...
// Fix voxel hights to 128 and the math floor for the biomes for original non-white stress test.
  const _VOXEL_HEIGHT = 1;
  const _OCEAN_LEVEL = Math.floor(_VOXEL_HEIGHT * 0.1);
  const _BEACH_LEVEL = _OCEAN_LEVEL + 2;
  const _SNOW_LEVEL = Math.floor(_VOXEL_HEIGHT * 0.1);
  const _MOUNTAIN_LEVEL = Math.floor(_VOXEL_HEIGHT * 0.1);
